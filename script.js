...
// ===================================
// Demo Canvas - Interactive Lighting
// ===================================
function initDemoCanvas() {
    ...
    // Create circular water wave panel
    const geometry = new THREE.CircleGeometry(5, 128, 128);

    // Convert to BufferGeometry with more detail
    const detailedGeometry = new THREE.PlaneGeometry(12, 12, 200, 200);
    ...
    const material = new THREE.ShaderMaterial({
        uniforms: {
            uTime: { value: 0 },
            uMouse: { value: new THREE.Vector2(0.5, 0.5) },
            uLightPos: { value: new THREE.Vector3(0, 0, 10) }
        },
        vertexShader: `
            uniform float uTime;
            ...
            void main() {
                ...
                // Multi-layer waves
                float wave1 = snoise(vec3(pos.x * 0.8, pos.y * 0.8, uTime * 0.2)) * 0.3;
                float wave2 = snoise(vec3(pos.x * 1.5 + 50.0, pos.y * 1.5, uTime * 0.3)) * 0.15;
                float wave3 = snoise(vec3(pos.x * 3.0, pos.y * 3.0 + 50.0, uTime * 0.4)) * 0.08;
                
                float elevation = (wave1 + wave2 + wave3) * mask;
                pos.z += elevation;
                ...
                float delta = 0.02;
                float px = snoise(vec3((pos.x + delta) * 0.8, pos.y * 0.8, uTime * 0.2));
                float nx = snoise(vec3((pos.x - delta) * 0.8, pos.y * 0.8, uTime * 0.2));
                float py = snoise(vec3(pos.x * 0.8, (pos.y + delta) * 0.8, uTime * 0.2));
                float ny = snoise(vec3(pos.x * 0.8, (pos.y - delta) * 0.8, uTime * 0.2));
                
                vec3 tangentX = normalize(vec3(2.0 * delta, 0.0, (px - nx) * mask * 0.3));
                vec3 tangentY = normalize(vec3(0.0, 2.0 * delta, (py - ny) * mask * 0.3));
                vNormal = normalize(cross(tangentX, tangentY));
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                vec3 lightPos = vec3((uMouse.x - 0.5) * 15.0, (uMouse.y - 0.5) * 15.0, 8.0);
                vec3 lightDir = normalize(lightPos - vPosition);
                vec3 viewDir = normalize(vec3(0.0, 0.0, 15.0) - vPosition);
                vec3 halfDir = normalize(lightDir + viewDir);
                
                // Multiple specular highlights
                float spec1 = pow(max(dot(vNormal, halfDir), 0.0), 32.0);
                float spec2 = pow(max(dot(vNormal, halfDir), 0.0), 128.0);
                float spec3 = pow(max(dot(vNormal, halfDir), 0.0), 256.0);
                ...
                vec3 silverColor = vec3(0.75, 0.75, 0.78);
                vec3 goldColor = vec3(0.83, 0.69, 0.22);
                
                vec3 color = baseColor;
                color += silverColor * spec1 * 0.6;
                color += goldColor * spec2 * 0.4;
                color += vec3(1.0) * spec3 * 0.8;
                color += silverColor * fresnel * 0.2;
                
                // Edge glow
                float edge = smoothstep(0.35, 0.45, dist) * mask;
                color += goldColor * edge * 0.3;
                
                // Rim lighting effect
                vec3 rimColor = vec3(0.2, 0.2, 0.25);
                color = mix(color, rimColor, smoothstep(0.3, 0.48, dist) * 0.5);
    ...
    const mesh = new THREE.Mesh(detailedGeometry, material);
    scene.add(mesh);

    camera.position.z = 10;

    // Add ambient lighting ring
    const ringGeometry = new THREE.RingGeometry(5.8, 6.2, 64);
